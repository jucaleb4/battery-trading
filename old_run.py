...
def validate(get_action, params):
    ...
    logger = SimpleLogger(params["fname"], params.get("env_mode", "default"))
...
def run_n_qlearn(n_cpu, params):
    ...
    def make_env(env_id: str, rank: int, params={}, seed: int=0):
        ...
        def _init() -> gym.Env:
            env = gym.make(
                "gym_examples/BatteryEnv-v0", 
                nhistory=params["nhistory"], 
                start_index=params["train_start"],
                end_index=params["end_index"],
                max_episode_steps=params["end_index"]-params["train_end"],
                mode=params["env_mode"], 
                more_data=params["more_data"],
            )
            (env, obs_scale, obs_shift) = get_normalized_env(env, params)
    ...
    fname_base += f"_more_data={params['more_data']}_seed={params['seed']}.csv"
    fname = os.path.join("logs", fname_base)
    params["fname"] = fname
    validate(env, params, get_action, obs_scale, obs_shift)
